...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        p = subprocess.Popen(['python',str(ROOT / "detect.py"), '--weights', str(last),
                                '--source',str(val_path), 
                                '--imgsz', str(imgsz), '--name', str(opt.name) ,'--save-txt','--exist-ok'])

        p.wait()
        detect_save_dir = str(Path(str(opt.project).replace('train','detect')) / opt.name / 'labels')
        ...
        r['wasserstein'] = wasserstein_distance(predict_labels, train_labels)
        if epoch < epochs - 1:
